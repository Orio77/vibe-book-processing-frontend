<script lang="ts">
    let { color1 = "hsl(315, 100%, 72%)", color2 = "hsl(227, 100%, 50%)" } =
        $props();

    // unique ID to prevent gradient conflicts when multiple books are rendered
    ...
    // random noise texture properties
...
<div class="relative w-full h-full flex flex-col justify-end overflow-hidden">
    <svg
        class="absolute inset-0 w-full h-full object-cover transition-transform duration-4000 group-hover:scale-150"
