<script lang="ts">
    ...
    let { sentence, isGrouped = false }: { sentence: Sentence; isGrouped?: boolean } = $props();
</script>
...
<span
    ...
    class="transition-all duration-150 rounded
        ...
        {!selectionStore.isSelectionMode && !isGrouped && !highlightsStore.chatSentenceIds.has(sentence.id) && highlightsStore.ideaSentenceIds.has(sentence.id) ? 'border-b-2 border-dashed border-accent cursor-pointer' : ''}
        {!selectionStore.isSelectionMode && highlightsStore.chatSentenceIds.has(sentence.id) ? 'border-b-2 border-dashed border-primary hover:bg-primary/10 cursor-pointer' : ''}
        {!selectionStore.isSelectionMode && !isGrouped && !highlightsStore.chatSentenceIds.has(sentence.id) && highlightsStore.ideaSentenceIds.has(sentence.id) ? 'border-b-2 border-dashed border-accent hover:bg-accent/10 cursor-pointer' : ''}"
    onclick={(e) => {
        if (selectionStore.isSelectionMode) return;
        if (highlightsStore.chatSentenceIds.has(sentence.id)) {
            highlightsStore.openTab('chat');
        } else if (!isGrouped && highlightsStore.ideaSentenceIds.has(sentence.id)) {
