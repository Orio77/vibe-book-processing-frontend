...
<div class="flex flex-col h-full overflow-y-auto p-4 space-y-6">
    ...
    {:else if state.isGenerating && state.summaries.length === 0}
        <div
            class="flex flex-col items-center justify-center py-10 space-y-4 text-base-content/60"
        >
    ...
    {:else}
        <div
            class="flex flex-col items-center justify-center py-10 text-center space-y-3 text-base-content/60"
        >
            <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
            ...
            <p class="text-xs">
                Click Generate to let AI summarize this chapter.
            </p>
        </div>
    {/if}
</div>
